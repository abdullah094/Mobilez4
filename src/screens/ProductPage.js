import { StyleSheet, Text, View,SafeAreaView, Pressable,Dimensions,ScrollView ,TouchableOpacity,Alert, Vibration,Platform,Linking} from 'react-native'
...
if(data){
  args.number = data.user.phone
      ...
      const CallWhatsappSms = () => (
        <View style={[{flexDirection:'row',width:"100%",alignItems:'center',justifyContent:'center',position:'absolute'},Platform.OS==='android'&&{bottom:25}]}>
        <TouchableOpacity style={styles.commmunication_buttons} onPress={()=>call(args).catch(console.error)}>
        <MaterialIcon name='phone' size={17} color={color.white}/>
          <Text style={styles.communication_buttons_text}>Call</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.commmunication_buttons} onPress={()=>{
...
backgroundColor:'white'
     ...
     <View>
        ...
        <View style={{flexDirection:'row',width:width-20,alignItems:'center',justifyContent:'space-between',marginTop:10}}>
        <Text style={[styles.heading,{fontSize:18,color:color.orange,marginBottom:0}]}>${data.price}</Text>
      ...
      <Text style={[styles.heading,{marginTop:0}]}>{data.brand} <Text>{data.model}</Text></Text>
      <View style={{flexDirection:'row',alignItems:'center',marginTop:5}}>
      <MaterialIcon name='location-on' size={13} color={color.orange}/>
      <Text style={[styles.heading,{fontSize:13,fontWeight:'normal',marginLeft:1,marginTop:0}]}>{data.user.city}</Text>
      </View>
     <TouchableOpacity onPress={Add_to_Wishlist}
      style={{marginVertical:10,padding:5,backgroundColor:'red',width:130,justifyContent:'center',alignItems:'center',borderRadius:10}}>
      <Text style={{color:color.white,fontWeight:'500',fontSize:15}}>Add to Wishlist</Text>
     </TouchableOpacity>
      </View>
      {/* Description */}
      <View style={{width:width-20,marginTop:10}}>
      <Text style={[styles.heading,{fontWeight:'500',fontSize:15,marginVertical:10,marginTop:25}]}>Description</Text>
      <Text style={{fontWeight:'200',color:color.black,}} numberOfLines={seemore}>{data.description}</Text>
     {/* See less see more button */}
     <TouchableOpacity onPress={()=>setSeemore(30)}>             
     {seemore===3? <Text>See more</Text>:<TouchableOpacity onPress={()=>setSeemore(3)}><Text>See less</Text></TouchableOpacity>} 
     </TouchableOpacity>
      </View>
