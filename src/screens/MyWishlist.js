      ...
      renderItem={({item}) => (
        <TouchableOpacity
          ...
          <Image
            ...
            resizeMode="contain"
            source={{uri: base_url + item.images.img}}
          />
          <View style={{width: 220, height: 120, marginLeft: 10}}>
            <Text style={{color: color.black, fontSize: 15}} numberOfLines={2}>
            ...
            {(item.product.ram || item.product.storage) && (
              <Text style={{color: 'gray', marginTop: 2, fontSize: 12}}>
                {item.product?.ram}GB | {item.product?.storage}GB
              </Text>
            )}
          </View>
        </TouchableOpacity>
